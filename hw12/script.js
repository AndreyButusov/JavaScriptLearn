    ...
    data.forEach(element => {
        list.insertAdjacentHTML('beforeend', `<div class="item1 ">
            ...
            <div class="item_text">
                ...
                <button class="item_button" data-id="${element.id}">Add to Cart</button>
            </div>
        </div>`)
    });
})
...
document.addEventListener('click', (event) => {
    if (event.target &&
        event.target.classList.contains('item_button')) {
        const productId =
